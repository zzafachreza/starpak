...
export default function Bayar({ navigation, route }) {
  ...
  console.warn('from checkout', route.params)
  ...
  const simpan = () => {
    ...
    setTimeout(() => {
      axios
        .post(urlAPI + '/1add_transaksi.php', data)
        .then(res => {
        ...
        {open && <View style={{ flex: 1, backgroundColor: colors.background1, marginVertical: 10, }}>
          <View
            ...
            <TouchableOpacity onPress={() => setOpen(false)} style={{
              ...
              <Icon type='ionicon' name='search' size={windowWidth / 30} color={colors.white} />
              <Text style={{
                color: colors.white,
                left: 2,
                fontFamily: fonts.secondary[600],
          ...
          <View
            ...
            <Text
              ...
              Rp. {new Intl.NumberFormat().format(route.params.harga_total + route.params.harga_admin)}
            </Text>
          </View>
